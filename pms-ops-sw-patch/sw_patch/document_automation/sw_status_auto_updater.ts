...
function main(workbook: ExcelScript.Workbook) {
  ...
  if (sw_product_cell_address && validation_product_cell_address && sw_release_date_cell_address && validation_release_date_cell_address && sw_version_cell_address && validation_version_cell_address && sw_patch_file_cell_address && validation_patch_file_cell_address) {
    ...
    console.log(`SW현황 "패치 파일(H)" 셀이 위치한 행과 열 ${sw_patch_file_cell_address.row}, ${sw_patch_file_cell_address.column}`);
    console.log(`최신 검증서 "패치 파일" 셀이 위치한 행과 열: ${validation_patch_file_cell_address.row}, ${validation_patch_file_cell_address.column}`);
    ...
    console.log("최신 검증서 시트 패치 파일 데이터:", validation_patch_file);
...
for (let product of common_values) {
  ...
  if (product_index_in_validation !== -1 && product_index_in_sw !== -1) {
    ...
    console.log(`제품명: ${product}, 최신 검증서 패치 파일: ${validation_patch_files}, SW현황 시트에 복사 완료`);

    console.log(`제품명: ${product}, 최신 검증서 버전: ${validation_product_version}, SW현황 시트에 복사 완료`);
  }
}
} else {
