function main(workbook: ExcelScript.Workbook) {
    ...
    let validation_patch_names = get_patch_names(lastest_validation_sheet, 'E');
  ...
  function update_sw_summary_sheet(
    ...
    Object.keys(validation_patch_names).forEach(patch_name_values => {
      if (sw_patch_names[patch_name_values] !== undefined) {
        ...
        sw_summary_sheet.getCell(row_in_sw_summary_sheet - 1, 6).setValue(lastest_validation_sheet.getCell(row_in_lastest_validation_sheet - 1, 7).getValue()); // H열(버전) 값 복사
        sw_summary_sheet.getCell(row_in_sw_summary_sheet - 1, 7).setValue(lastest_validation_sheet.getCell(row_in_lastest_validation_sheet - 1, 8).getValue()); // I열(패치 파일명) 값 복사
